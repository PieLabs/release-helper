...
let githubToken =
  args["github-token"] || args.githubToken || process.env.GITHUB_TOKEN;
...
exports.init = function(gulp, dir = process.cwd()) {
  ...
  gulp.task("commit-release-changes", () =>
    commitChanges(`[release] set version number to ${getPackageJsonVersion()}`)
  );
  gulp.task("commit-bump-changes", () =>
    commitChanges(`[bump] set version number to ${getPackageJsonVersion()}`)
  );
  ...
  let push = (origin, name, opts) => done =>
    git.push(origin, name, opts || {}, done);
    ...
    request(url, (err, response, body) => {
      if (result(body).status === "good") {
        ...
        if (skipGithub) {
          gutil.log(
            gutil.colors.yellow(
              "github is down but --skip-github is set, proceeding..."
            )
          );
          done();
        } else {
